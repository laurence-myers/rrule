...
RRule.optionsToString = function(options) {
    var key, keys, value, strValues, exdate, pairs = [], output = '';
	...
	*/
    ...
    _iter: function(iterResult) {
        ...
        */
        ...
        var exdate_set = {};
        _.each(exdate, function (date) {
            exdate_set[date.getTime()] = true;
            // HACK:
            // This lib seems to apply Daylights Savings, which means incrementing by a month can add (or maybe remove)
            //  an hour. This breaks the comparison to excluded dates, which are only ever at midnight.
            // I only care about excluding dates, not times, so for every excluded date, I'm adding two more entries,
            //  plus or minus 1 hour either side of the excluded date's time.
            var hourDiff = 1 * 60 * 60 * 1000; // hour * min * sec * ms
            exdate_set[date.getTime() + hourDiff] = true;
            exdate_set[date.getTime() - hourDiff] = true;
        });
...
RRule.parseString = function(rfcString) {
    ...
    var i, j, key, attr, value,
        options = {};

    if (rfcString.substr(0, 6) === 'EXDATE') {
        value = rfcString.split('\r\n');
        rfcString = value[1];
        value = value[0];
        ...
        options.exdate = value.sort();
    }

    if (rfcString.substr(0, 6) === 'RRULE:') {
        rfcString = rfcString.substr(6);
    }
